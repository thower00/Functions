...
// -------------------------------------------------------
    ...
    let
        ...
        if Contains <> null then
            Table.SelectRows(FilteredRows, each Text.Contains([Name], Contains))
        else
            FilteredRows,
        ...
        if Newest then
            Table.Sort(FilteredContains,{{"Date modified", Order.Descending}})
        else
            Table.Sort(FilteredContains,{{"Date modified", Order.Ascending}}),
