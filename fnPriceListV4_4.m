...
2021-11-24    tswm00    V4.1: Indatatabellen kan utgöras av tabell eller text (namn på DB query)
2021-11-29    tswm00    V4.2: Returnerar också CA-Description och CSA-Description
2021-12-02    tswm00    V4.3: Utökar med närmast datum urval för TMSMapping och TMSReMapping tabellerna.
                              Bygger på nya union queries i databasen (qTMSPriceListAll-V1, qTMSMappingAll-V1)
2021-12-09    tswm00    V4.4: Möjliggör att få ut alla entries i prislistan utan datum urval genom att sätta Date_p = 0
*/
// -------------------------------------------------------
// Beskrivning
// -------------------------------------------------------
...
Parametrar:
    IndataTbl_p                 Indatatabell med nyckel som skall översättas till TMS och returnera priser för dessa
                                Eller referens till qSpecificComponentModel i DB som utgör indata
                                Eller null (indata utgörs av prislistan)
    IndataKeyCol_p              Indatatabellens nyckelkolumn, eller null om indatatabell saknas enligt ovan
    ...
    Date_p                      Prislistedatum, senaste priset närmast före eller lika med detta datum. Ange 0 för att få ut alla entries
...
UseCase3:
-----------------------------------------------------------------------------
Ingen indatatabell. Vill ha ut mappning från specifika begrepp genom att referens till qSpecificComponentModel skickas med som
indataparameter.
Prislistan från databasen enligt angivna parametrar (Date, DateMin, DateMax)
Referns (text) till qSpecificComponentModel i databasen sätts som indata.
TMSMapping KAN (och bör) användas om TMSMappingDBQuery_p skickas med.
TMSReMapping KAN användas om TMSReMappingDBQuery_pskickas med.

UseCase4:
-----------------------------------------------------------------------------
Alla entries i prislistan utan datumurval.
Date_p sätts till 0.
Om man skickar med parametrar för TMSMapping och/eller TMSReMapping så kan det leda till oförutsägbara resultat.
...
/*
TestScenario = 8,
//    1: Allt:  Bara prislista utan indatatabell. Utan DateMin/DateMax
//    2: O365:  Med indatatabell. Med TMSReMapping. Utan DateMin/DateMax
//    3: O365:  Med indatatabell. Med TMSReMapping. Med DateMin/DateMax
//    4: NÄT:   Bara prislista utan indatatabell. Med DateMin/DateMax
//    5: Allt:  Bara prislista utan indatatabell. Med DateMin/DateMax
//    6: Allt:  qSpecificComponent skickas med som text referens
//    7: Nät:   Alla poster i prislistan utan datumurval
//    8: Nät:   Connections
...
//*/
        ...
        // Manage parameters 
        // ---------------------------------------------------
        Is_ReturnAllPriceListEntries = Date_p = DateTime.From(0),
        Is_IndataPriceList = IndataTbl_p = null,
        Is_IndataTable = Value.Is(IndataTbl_p, type table),
        Is_IndataDBquery = Value.Is(IndataTbl_p, type text),

        IndataTbl = 
        if Is_IndataPriceList then
        // Indatabellen är tom så sätt prislistan som indata
            IndataTblFromPriceList
        else if Is_IndataDBquery then
            IndataTblFromSpecificComponentModel
        else
            IndataTbl_p,
        ...
        // Indata utgörs av prislistan. Sätt rätt kolumn
            "StCM-Denomination"
        else if Is_IndataTable and IndataKeyCol_p <> null  then
            IndataKeyCol_p
        else if Is_IndataDBquery and IndataKeyCol_p <> null  then
            IndataKeyCol_p
        else
        ...
        // ---------------------------------------------------
        // Create indata table from qSpecificComponentModel 
        // ---------------------------------------------------
        IndataTblFromSpecificComponentModel =
        let
            SourceFile = Access.Database(File.Contents(PricelistDBFile)),
            SourceData = SourceFile{[Schema="",Item=IndataTbl_p]}[Data],
            UppercaseText = Table.TransformColumns(SourceData,{{"CA-Description", Text.Upper, type text}, {"CSA-Description", Text.Upper, type text}, {"SpCM-Denomination", Text.Upper, type text}}),
            SelectServiceArea = 
            if ServiceArea <> null then
                Table.SelectRows(UppercaseText, each ([#"CA-Description"] = ServiceArea))
            else
                UppercaseText,

            SelectWithinInterval = 
            if Date <> null then
                Table.SelectRows(SelectServiceArea, each (Date >= [ValidFromDate]) and (Date <= [ValidToDate]))
            else
                SelectServiceArea,

            RemovedOtherColumns = Table.SelectColumns(SelectWithinInterval,{"SpCM-Denomination", "CA-Description", "CSA-Description"})
        in
            RemovedOtherColumns,
        ...
        let
            ...
            PriceList =
            if Is_ReturnAllPriceListEntries then
                SelectMaxDate
            else
                let
        ...
        let
            ...
            UppercasedText = Table.TransformColumns(ExcludeInactive,{{"SpCM-Denomination", Text.Upper, type text}, {"StCM-Denomination", Text.Upper, type text}, {"CA-Description", Text.Upper, type text}, {"CSA-Description", Text.Upper, type text}}),
            ...
            DateValue = Table.AddColumn(SelectWithinInterval, "DateValue", each [ValidFromDate]),
            UsedDateValue = Table.AddColumn(DateValue, "UsedDateValue", each Date),
            ChangedTypeDateValue = Table.TransformColumnTypes(UsedDateValue,{{"DateValue", type number}, {"UsedDateValue", type number}}),
            ...
            Grouped = Table.Group(RemoveFutureDates, {"SpCM-Denomination"}, {{"AllData", each _, type table}, {"MaxDiffDateValue", each List.Max([DiffDateValue]), type number}}),
            SelectedMax = Table.AddColumn(Grouped, "Custom", (x) => Table.SelectRows(x[AllData], each [DiffDateValue] = x[MaxDiffDateValue])),
            RemoveColumns1 = Table.RemoveColumns(SelectedMax,{"AllData", "MaxDiffDateValue"}),
            ExpandCustom = Table.ExpandTableColumn(RemoveColumns1, "Custom", {"StCM-Denomination", "CA-Description", "CSA-Description", "ValidFromDate", "ValidToDate", "DateValue", "UsedDateValue", "DiffDateValue"}, {"StCM-Denomination", "CA-Description", "CSA-Description", "ValidFromDate", "ValidToDate", "DateValue", "UsedDateValue", "DiffDateValue"}),
            RemovedDuplicates = Table.Distinct(ExpandCustom, {"SpCM-Denomination"}),
            RenameDate = Table.RenameColumns(RemovedDuplicates,{{"ValidFromDate", "MapFromDate"}, {"ValidToDate", "MapToDate"}}),
            OrganizeColumns = Table.SelectColumns(RenameDate,{"SpCM-Denomination", "StCM-Denomination", "CA-Description", "CSA-Description", "MapFromDate", "MapToDate"}),
            ChangeType = Table.TransformColumnTypes(OrganizeColumns,{{"SpCM-Denomination", type text}, {"StCM-Denomination", type text}, {"CA-Description", type text}, {"CSA-Description", type text}, {"MapFromDate", type datetime}, {"MapToDate", type datetime}})
        in
            ChangeType,
        ...
        let
            ...
            SelectWithinInterval = 
            if Date <> null then
                Table.SelectRows(SelectServiceArea, each (Date >= [ReMapFromDate]) and (Date <= [ReMapToDate]))
            else
                SelectServiceArea,

            DateValue = Table.AddColumn(SelectWithinInterval, "DateValue", each [ReMapFromDate]),
            UsedDateValue = Table.AddColumn(DateValue, "UsedDateValue", each Date),
            ChangedTypeDateValue = Table.TransformColumnTypes(UsedDateValue,{{"DateValue", type number}, {"UsedDateValue", type number}}),
            ...
            Grouped = Table.Group(RemoveFutureDates, {"ReMapFromComponent"}, {{"AllData", each _, type table}, {"MaxDiffDateValue", each List.Max([DiffDateValue]), type number}}),
            SelectedMax = Table.AddColumn(Grouped, "Custom", (x) => Table.SelectRows(x[AllData], each [DiffDateValue] = x[MaxDiffDateValue])),
            RemoveColumns1 = Table.RemoveColumns(SelectedMax,{"AllData", "MaxDiffDateValue"}),
            ExpandCustom = Table.ExpandTableColumn(RemoveColumns1, "Custom", {"Domain", "Subdomain", "fReMapFromComponent", "ReMapFromComponent", "fReMapToComponent", "ReMapToComponent", "ReMapFromDate", "ReMapToDate", "Comment", "DateValue", "UsedDateValue", "DiffDateValue"}, {"Domain", "Subdomain", "fReMapFromComponent", "ReMapFromComponent.1", "fReMapToComponent", "ReMapToComponent", "ReMapFromDate", "ReMapToDate", "Comment", "DateValue", "UsedDateValue", "DiffDateValue"}),
            RemovedDuplicates = Table.Distinct(ExpandCustom, {"fReMapFromComponent"}),
            OrganizeColumns = Table.SelectColumns(RemovedDuplicates,{"Domain", "Subdomain", "fReMapFromComponent", "ReMapFromComponent", "fReMapToComponent", "ReMapToComponent", "ReMapFromDate", "ReMapToDate", "Comment"}),
            ChangeType = Table.TransformColumnTypes(OrganizeColumns,{{"Domain", type text}, {"Subdomain", type text}, {"ReMapFromComponent", type text}, {"ReMapToComponent", type text}, {"Comment", type text}, {"ReMapFromDate", type datetime}, {"ReMapToDate", type datetime}})
        in
            ChangeType,
        ...
        let
            ...
            ExpandTMSMapping = Table.ExpandTableColumn(MergeTMSMapping, "TMSMapping", {"StCM-Denomination", "MapFromDate", "MapToDate"}, {"fnPriceList.TMSMappedToTEMP", "fnPriceList.MapFromDate", "fnPriceList.MapToDate"}),
            ...
            ChangeType = Table.TransformColumnTypes(fnPriceListTMSMapped,{{"fnPriceList.TMSMapped", type logical}}),

            fnPriceListTMSMappedTo = Table.AddColumn(ChangeType, "fnPriceList.TMSMappedTo", each
        ...
        let
            ...
            ChangedType = Table.TransformColumnTypes(fnPriceListTMSMapped,{{"fnPriceList.TMSMapped", type logical}}),
            fnPriceListTMSMappedTo = Table.AddColumn(ChangedType, "fnPriceList.TMSMappedTo", each [#"StCM-Denomination"], type text),
            fnPriceListMapFromDate = Table.AddColumn(fnPriceListTMSMappedTo, "fnPriceList.MapFromDate", each null, type datetime),
            fnPriceListMapToDate = Table.AddColumn(fnPriceListMapFromDate, "fnPriceList.MapToDate", each null, type datetime),
            RemoveOtherColumns = Table.SelectColumns(fnPriceListMapToDate,{"StCM-Denomination", "CA-Description", "CSA-Description", "fnPriceList.TMSMapped", "fnPriceList.TMSMappedTo", "fnPriceList.MapFromDate", "fnPriceList.MapToDate"})
        in
            RemoveOtherColumns,
        ...
        let
            ...
            ExpandTMSReMapping = Table.ExpandTableColumn(MergeTMSReMapping, "TMSReMapping", {"ReMapToComponent", "ReMapFromDate", "ReMapToDate"}, {"fnPriceList.TMSReMappedToTEMP", "fnPriceList.ReMapFromDate", "fnPriceList.ReMapToDate"}),
            ...
            ChangeType = Table.TransformColumnTypes(fnPriceListTMSReMapped,{{"fnPriceList.TMSReMapped", type logical}}),
            fnPriceListTMSReMappedTo = Table.AddColumn(ChangeType, "fnPriceList.TMSReMappedTo", each
        ...
        let
            ...
            fnPriceListReMapFromDate = Table.AddColumn(fnPriceListTMSReMapped, "fnPriceList.ReMapFromDate", each null, type datetime),
            fnPriceListReMapToDate = Table.AddColumn(fnPriceListReMapFromDate, "fnPriceList.ReMapToDate", each null, type datetime),
            ChangedType = Table.TransformColumnTypes(fnPriceListReMapToDate,{{"fnPriceList.TMSReMapped", type logical}}),
            fnPriceListTMSReMappedTo = Table.AddColumn(ChangedType, "fnPriceList.TMSReMappedTo", each [fnPriceList.TMSMappedTo], type text)
        in
        ...
        ExpandPriceList = Table.ExpandTableColumn(MergePriceList, "PriceList", {"CA-Description", "CSA-Description", "StCM-Denomination", "StCMP-CommonCostKey", "StCMP-CommonCostValue", "StCMP-CommonCostDepTime", "StCMP-TOCostKey", "StCMP-TOCostValue", "StCMP-TOCostDepTime", "StCMP-OneTimeCost", "StCMP-OneTimeCostDepTime", "StCMP-PurchaseCost", "StCMP-PurchaseCostDepTime", "StCMP-InstallationCost", "StCMP-InstallationCostDepTime", "StCMP-OperatingCost", "StCMP-OperatingCostDepTime", "MonthlyCost", "StCMP-ChangeDate", "StCMP-ChangeInfo"}, {"PriceList.CA-Description", "PriceList.CSA-Description", "PriceList.StCM-Denomination", "PriceList.StCMP-CommonCostKey", "PriceList.StCMP-CommonCostValue", "PriceList.StCMP-CommonCostDepTime", "PriceList.StCMP-TOCostKey", "PriceList.StCMP-TOCostValue", "PriceList.StCMP-TOCostDepTime", "PriceList.StCMP-OneTimeCost", "PriceList.StCMP-OneTimeCostDepTime", "PriceList.StCMP-PurchaseCost", "PriceList.StCMP-PurchaseCostDepTime", "PriceList.StCMP-InstallationCost", "PriceList.StCMP-InstallationCostDepTime", "PriceList.StCMP-OperatingCost", "PriceList.StCMP-OperatingCostDepTime", "PriceList.MonthlyCost", "PriceList.StCMP-ChangeDate", "PriceList.StCMP-ChangeInfo"}),
        ...
        ChangedType = Table.TransformColumnTypes(fnPriceListPriceListValid,{{"fnPriceList.PriceListValid", type logical}}),
        NoTMSFoundInPriceList = Table.ReplaceValue(ChangedType,null,ERR_TMSNotFoundInPriceList,Replacer.ReplaceValue,{"PriceList.StCM-Denomination"}),
        ...
        if Is_ReturnAllPriceListEntries then
            let
                CheckDuplicatesColumns = Table.AddColumn(NoTMSFoundInPriceList, "CheckDuplicatesColumns", each [#"PriceList.StCM-Denomination"] & Text.From([#"PriceList.StCMP-ChangeDate"]), type text),
                RemovedDuplicates = Table.Distinct(CheckDuplicatesColumns, {"CheckDuplicatesColumns"}),
                RemovedColumns = Table.RemoveColumns(RemovedDuplicates,{"CheckDuplicatesColumns"}),
                SortedRows = Table.Sort(RemovedColumns,{{"PriceList.CA-Description", Order.Ascending}, {"PriceList.CSA-Description", Order.Ascending}, {"PriceList.StCM-Denomination", Order.Ascending}, {"PriceList.StCMP-ChangeDate", Order.Ascending}})
            in
                SortedRows
        else
            NoTMSFoundInPriceList,
